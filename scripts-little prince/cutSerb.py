...
for chapterIndex in range(len(chapters)):
	...
	mp3Path = workDirectory+"/"+str(chapterIndex).zfill(2)+".wav"

	textPath = workDirectory+"/"+thisFilename
	jsonPath = workDirectory+"/"+thisFilename+".json"
	parameterString = '"task_language=fr|os_task_file_format=json|is_text_type=plain|task_adjust_boundary_algorithm=beforenext|task_adjust_boundary_beforenext_value=0.200"'
